...
## 前段階ファイル読込
- `.claude_workflow/complete.md` を読み込み済み
...
## 背景
- 現在の `shooting3Dterrer4` は、ゲーム開始後にコントローラー操作で武器切替できない。
- Stage2 終了後の GameOver 押下時に、終了時挙動をより明示的に VR モード解除へ寄せたい。
- ステージ時間調整、弾数制限、弾薬補給ドロップ（特定モデル撃破時）を追加したい。

## 要件（ユーザー依頼）
1. ゲームスタート後も、グリップ / A / B で武器切替できること。
2. Stage2 終了後に GameOver ボタンを押すと VR モードが解除されること。
3. 時間調整:
  - Stage1: 10秒延長（合計 100 秒）
  - Stage2: 10秒短縮（合計 80 秒）
  - ボス出現は引き続き残り15秒
4. 発射可能ボールに弾数制限を導入し、残弾（Gun1/Gun2）をアイコン付きで常時表示すること。
5. 初期残弾は Gun1=20, Gun2=20。
6. `01.glb` 撃破時:
  - pokeball が出現し、カメラ方向へ飛来
  - 現在未使用側の武器残弾を +5
7. `06.glb` 撃破時:
  - pokeball が出現し、カメラ方向へ飛来
  - 現在未使用側の武器残弾を +10
...
## 現時点の実装把握
- ステージ時間は `window.STAGE_CONFIG` で管理（現状 Stage1/2 ともに 90 秒）。
- ボス出現条件は `window.gameTimeLeft === 15`。
- 武器切替 `switchGun()` は `start-menu` 内にあり、現状は `window.gameStarted` 中に return している。
- 残弾管理と UI 表示は未実装。
- モデル撃破判定は `hit-box` コンポーネントで実施。

## 不明点（設計前に確認が必要）
- なし（2026-06-09時点で全項目確認済み）

## 確認済み回答（2026-06-09）
- 残弾 UI: Gun1/Gun2 の両方を常時表示
- 残弾アイコン: `public/cg/pokeball_icon05.png`, `public/cg/pokeball_icon06.png` を使用
- 残弾 0 時: 武器切替は可能、発射のみ不可
- 弾数上限: 上限なし
- Stage2 GameOver 押下後: VR 解除のみ（ページは維持）
- 補給ボール加算タイミング: カメラ近くに到達したら加算

## 成功基準（確定版は不明点回答後に更新）
- 指定された操作・時間・残弾・補給仕様が反映される。
- 既存のスコア計算、ボス出現条件（残り15秒）、ステージ遷移の基本挙動を壊さない。
- 変更対象 PHP/Blade ファイルで `php -l` エラーなし。
