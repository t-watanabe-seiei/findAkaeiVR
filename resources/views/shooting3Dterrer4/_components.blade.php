<script>
    ...
    AFRAME.registerComponent('start-menu', {
        ...
        activateModels: function(sceneEl) {
            const cfg = window.STAGE_CONFIG[window.currentStage];
            const initPats = [
                { startPos: {x:-5,y:-0.15,z:-10}, speed:0.4, useCamera:true,  waitTime:4000 },
                { startPos: {x: 5,y:-0.15,z:-10}, speed:0.3, useCamera:true,  waitTime:4000 },
                { startPos: {x: 0,y:-0.15,z:-12}, speed:0.2, useCamera:true,  waitTime:4000 },
                { startPos: {x:-6,y:-0.15,z: -8}, endPos:{x:6,y:0,z:-8},  speed:0.35, useCamera:false, waitTime:4000 },
                { startPos: {x: 6,y:-0.15,z: -8}, endPos:{x:-6,y:0,z:-8}, speed:0.35, useCamera:false, waitTime:4000 },
            ];
    ...
    AFRAME.registerComponent('hit-box', {
        ...
        createNewModel: function(modelId, gltfSrc, sceneEl) {
            if (window.gameEnded || !window.gameStarted) return;
            const pats = [
                { startPos: {x:-3,y:-0.15,z:-3}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x: 0,y:-0.15,z:-4}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x: 3,y:-0.15,z:-3}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x: 4,y:-0.15,z: 0}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x: 5,y:-0.15,z: 3}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x:-6,y:-0.15,z: 0}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x: 0,y:-0.15,z: 4}, speed:0.15, useCamera:true, waitTime:4000 },
                { startPos: {x: 3,y:-0.15,z: 6}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x:-4,y:-0.15,z: 6}, speed:0.25, useCamera:true, waitTime:4000 },
                { startPos: {x:-7,y:-0.15,z: 2}, speed:0.25, useCamera:true, waitTime:4000 },
            ];
            const speedMult = window.currentStage === 2 ? 1.2 : 1.0;
