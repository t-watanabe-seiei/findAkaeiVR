...
<head>
    ...
    <script>  
        ...
        // 説明（日本語）:
        //  - モデル描画完了後（'model-loaded'）に、モデルの THREE.js のバウンディングボックスを取得
        //  - 親エンティティ内の hit-box（円柱）に対応する THREE.js のバウンディングボックスを取得
        //  - hitbox に収まる最小のスケール比を算出して、モデルに適用する
        //  - 模型の重心位置をhit-boxの中心に合わせる（必要に応じてYオフセットを調整）
        AFRAME.registerComponent('fit-to-hitbox', {
            schema: {
                // optional margin to keep model a bit smaller than hitbox (0.0 - 1.0)
                margin: { type: 'number', default: 0.85 }
            },
            init: function() {
                this.onModelLoaded = this.onModelLoaded.bind(this);
                this.el.addEventListener('model-loaded', this.onModelLoaded);
                // safety retry in case model already loaded before component inits
                setTimeout(() => {
            ...
            onModelLoaded: function() {
                try {
                    ...
                    if (!parent) return;
                    const hitBoxEl = parent.querySelector('[hit-box]');
                    if (!hitBoxEl) return; // hitboxがない場合は自動スケールしない

                    const hitMesh = hitBoxEl.getObject3D('mesh') || hitBoxEl.object3D;
                    if (!hitMesh) {
                        // hitbox のメッシュがまだなければ少し待ってリトライ
                        setTimeout(() => this.onModelLoaded(), 50);
                        return;
                    }

                    // THREE.Box3 を使ってバウンディングボックスを取得
                    const boxModel = new THREE.Box3();
                    boxModel.setFromObject(modelObj);
                    const modelSize = boxModel.getSize(new THREE.Vector3());

                    const boxHit = new THREE.Box3();
                    boxHit.setFromObject(hitMesh);
                    const hitSize = boxHit.getSize(new THREE.Vector3());

                    // バウンディングの大きさがゼロの場合はスキップ
                    if ((modelSize.x <= 0 && modelSize.z <= 0 && modelSize.y <= 0) ||
                        (hitSize.x <= 0 && hitSize.z <= 0 && hitSize.y <= 0)) {
                        return;
                    }

                    // 水平方向 (x,z) と高さ (y) を考慮し、最小スケール比で合わせる
                    const scaleX = (hitSize.x / modelSize.x) || Infinity;
                    const scaleZ = (hitSize.z / modelSize.z) || Infinity;
                    const scaleY = (hitSize.y / modelSize.y) || Infinity;
                    const targetScale = Math.min(scaleX, scaleZ, scaleY) * (this.data.margin || 0.85);
                    if (!isFinite(targetScale) || targetScale <= 0) return;

                    // 現在のscaleに対して乗算するのではなく、モデルのローカル scale を targetScale に直接設定
                    this.el.setAttribute('scale', `${targetScale} ${targetScale} ${targetScale}`);

                    // 中心をhitboxの中心に合わせる（モデルの原点位置による誤差があれば補正）
                    // modelがバウンディングボックスの中心からずれている場合は、重心位置を調整
                    // ただしGLTFの原点によっては移動が必要ないケースもあるため最小限の補正に留める
                    const modelCenter = boxModel.getCenter(new THREE.Vector3());
                    const hitCenter = boxHit.getCenter(new THREE.Vector3());
                    // 世界座標での差分をローカル座標に変換して適用
                    const worldModelCenter = modelCenter;
                    const worldHitCenter = hitCenter;
                    const elWorldPos = new THREE.Vector3();
                    this.el.object3D.getWorldPosition(elWorldPos);
                    const offset = new THREE.Vector3().subVectors(worldHitCenter, worldModelCenter);
                    // Offsetをローカルに変換
                    const parentInv = new THREE.Matrix4().copy(this.el.object3D.parent.matrixWorld).invert();
                    offset.applyMatrix4(parentInv);
                    // 過度な移動は避ける（Yの微調整のみを許容）
                    this.el.object3D.position.add(new THREE.Vector3(offset.x, offset.y, offset.z));

                    console.log('fit-to-hitbox:', this.el, 'applied scale', targetScale.toFixed(3));
                } catch (err) {
