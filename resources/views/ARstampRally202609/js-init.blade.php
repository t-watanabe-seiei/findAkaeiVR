...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setGuideLanguage(lang) {
        ...
        if (lang === 'jp') {
            ...
            if (stepPrize) stepPrize.innerHTML = `
                <div class="step-text">
                    <strong>景品交換</strong>
                    <p>会場で11種類以上のマーカーを集めると景品と交換できます。</p>
            ...
            if (stepPrize) stepPrize.innerHTML = `
                <div class="step-text">
                    ...
                    <p>Collect 11 or more markers in the venue to exchange for a prize.</p>
    ...
    if (confirmYes) {
        confirmYes.addEventListener('click', function (e) {
            ...
            localStorage.removeItem('ar-stamp-rally-202609');
            localStorage.removeItem('ar-captured-animals-202609');
    ...
    if (scene) {
        ...
        scene.addEventListener('arjs-video-loaded', function () {
            ...
            try { var ce = document.getElementById('camera-error'); if (ce) ce.style.display = 'none'; window._pendingCameraError = false; } catch (e) {} // 2026-09-11 fix: 二重防御（202610 と同一）
    ...
    // 2026-09-11 fix: 低スペックiOS（iPhone7/SE3 等）は AR.js のコールドスタートが遅いため監視タイムアウトを延長。iPadOS 13+（Mac UA偽装）も iOS として扱う。
