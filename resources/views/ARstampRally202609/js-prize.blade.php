        ...
        function getUserId202609() {
            var storageKey = 'ar-user-id-202609';
            var cookieName = 'ar_user_id_202609';
            var userId = localStorage.getItem(storageKey) || CookieHelper202609.get(cookieName);
            if (userId) {
                localStorage.setItem(storageKey, userId);
                CookieHelper202609.set(cookieName, userId);
                UserIdDB202609.saveUserId(userId);
                return Promise.resolve(userId);
            }
            return UserIdDB202609.getUserId().then(function (uid) {
                if (uid) {
                    localStorage.setItem(storageKey, uid);
                    CookieHelper202609.set(cookieName, uid);
                    return uid;
                }
                var newId = generateUUID202609();
                localStorage.setItem(storageKey, newId);
                CookieHelper202609.set(cookieName, newId);
                UserIdDB202609.saveUserId(newId);
                return newId;
            });
        }
        ...
        function generateFingerprint() {
            return getUserId202609().then(function (userId) {
                var data = [
                    userId,
                    navigator.userAgent,
                    navigator.language,
