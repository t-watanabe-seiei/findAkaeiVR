    ...
    <script>
                ...
                if (gameActive && actualNumber === currentNextNumber) {
                    ...
                    animationPromise.then(() => {
                        try {
                            ...
                            console.log('collectAndMarkWithRetry (seq success) returned', saved, 'for', stampId);

                            // 収集情報に番号を追記
                            try {
                                const stamps = getCollectedStamps();
                                stamps[stampId] = stamps[stampId] || {};
                                stamps[stampId].collectedAt = new Date().toISOString();
                                stamps[stampId].name = (STAMPS[stampId] && STAMPS[stampId].name) ? STAMPS[stampId].name : stampId;
                                stamps[stampId].number = actualNumber;
                                saveCollectedStamps(stamps);
                            } catch (e) { console.warn('Failed to annotate collected stamp with number', e); }
                            ...
                            updateStampBadge();
                            // スタンプ帳を更新（ボタンが開いているなら反映）
                            try { if (document.getElementById('stamp-book-modal').style.display === 'block') showStampBook(); } catch(e){}
                    ...
                    // 非シーケンシャル（ゲーム非アクティブ）: fallback original animation handling
                    ...
                    try { destroyAndFreeEntity(this.el); } catch (e) { console.warn('Immediate destroy failed', e); }
                    ...
                    animationPromise.then(() => {
                        ...
                        } catch (e) { console.warn('Post-animation fallback handling failed', e); }
                    });
                }
            },
            },
        ...
        AFRAME.registerComponent('lazy-model', {
            ...
            onMarkerFound: function() {
                ...
                // 番号ラベルを表示
                try {
                    const stampId = (this.el.id || '').replace('-model','');
                    if (stampId) showNumberLabel(stampId);
                } catch (e) { /* ignore */ }
            },
            onMarkerLost: function() {
                ...
                try {
                    const stampId = (this.el.id || '').replace('-model','');
                    if (stampId) hideNumberLabel(stampId);
                } catch (e) { /* ignore */ }
...
<body>
    ...
    <script>
        ...
        // Game BGM & hit sound
        const soundBgm = new Audio("{{ asset('cg/sound_bgm07.mp3') }}");
        soundBgm.loop = true;
        ...
        let numberToStamp = {}; // number -> stampId
        ...
        function showPenaltyMessage() {
            const el = document.getElementById('penalty-message');
            if (!el) return;
            el.style.display = '';
        ...
        function assignRandomNumbers() {
            ...
            const nums = [];
            for (let i = 1; i <= TOTAL_STAMP_SLOTS; i++) nums.push(i);
            ...
            for (let i = 0; i < stampKeys.length; i++) {
                const id = stampKeys[i];
                const n = nums[i];
                assignedNumbers[id] = n;
                numberToStamp[n] = id;
            }
            // persist for session
            sessionStorage.setItem('ar-assigned-numbers', JSON.stringify(assignedNumbers));
            console.log('Assigned numbers:', assignedNumbers);
        }

        function loadAssignedNumbersIfPresent() {
            const s = sessionStorage.getItem('ar-assigned-numbers');
            if (s) {
                try { assignedNumbers = JSON.parse(s); for (const k in assignedNumbers) { numberToStamp[assignedNumbers[k]] = k; } } catch(e) { assignedNumbers = {}; numberToStamp = {}; }
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 各モデルに番号ラベル（a-entity）を追加して非表示にしておく
            const stampKeysInit = Object.keys(STAMPS).slice(0, TOTAL_STAMP_SLOTS);
            stampKeysInit.forEach(id => {
                const model = document.getElementById(id + '-model');
                if (!model) return;
                if (model.querySelector('.number-label')) return;
                const label = document.createElement('a-entity');
                label.className = 'number-label';
                label.setAttribute('text', 'value: ; align: center; color: #fff; width: 2');
                label.setAttribute('geometry', 'primitive: plane; width: 0.7; height: 0.35');
                label.setAttribute('material', 'color:#000;opacity:0.6;side:double');
                // 位置はモデルにより調整の余地あり
