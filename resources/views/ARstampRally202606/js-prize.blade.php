        ...
        function getUserId202606() {
            var storageKey = 'ar-user-id-202606';
            var cookieName = 'ar_user_id_202606';
            var userId = localStorage.getItem(storageKey) || CookieHelper202606.get(cookieName);
            if (userId) {
                localStorage.setItem(storageKey, userId);
                CookieHelper202606.set(cookieName, userId);
                UserIdDB202606.saveUserId(userId);
                return Promise.resolve(userId);
            }
            return UserIdDB202606.getUserId().then(function (uid) {
                if (uid) {
                    localStorage.setItem(storageKey, uid);
                    CookieHelper202606.set(cookieName, uid);
                    return uid;
                }
                var newId = generateUUID202606();
                localStorage.setItem(storageKey, newId);
                CookieHelper202606.set(cookieName, newId);
                UserIdDB202606.saveUserId(newId);
                return newId;
            });
        }
        ...
        function generateFingerprint() {
            return getUserId202606().then(function (userId) {
                var data = [
                    userId,
                    navigator.userAgent,
                    navigator.language,
        ...
        function recordMarkerDetection(markerId, markerName) {
            ...
            var cacheKey = 'marker-scan-cache-202606-' + markerId + '-' + today;
            ...
            checkPrizeExchangeStatus().then(function (serverStatus) {
                ...
                return generateFingerprint().then(function (fp) {
                    return fetch('{{ url("/api/exchange-prize") }}', {
                        ...
                        if (data.success) {
                            localStorage.setItem('ar-prize-exchanged-202606', 'true');
                            localStorage.setItem('ar-prize-code-202606', data.prizeCode);
                            updatePrizeButton();
