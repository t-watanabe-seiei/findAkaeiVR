<script>
    ...
    AFRAME.registerComponent('pokeball-throwable', {
        ...
        tick: function (time, deltaTime) {
            ...
            // ---- ギャラリー hitbox チェック（スタンプ取得なし） ----
            if (typeof window.allGalleryHitboxes !== 'undefined') {
                for (var gi = 0; gi < window.allGalleryHitboxes.length; gi++) {
                    const ghb = window.allGalleryHitboxes[gi];
                    if (!ghb.el.object3D.visible) continue;
                    if (ghb.el.parentElement && ghb.el.parentElement.object3D && !ghb.el.parentElement.object3D.visible) continue;

                    var isGalleryHit = false;
                    const ballPos2 = this.el.object3D.getWorldPosition(new THREE.Vector3());
                    if (ghb.checkCollision(ballPos2)) {
                        isGalleryHit = true;
                    } else if (ghb.checkIntersection) {
                        if (ghb.checkIntersection(ray, distance)) {
                            isGalleryHit = true;
                        }
                    }

                    if (isGalleryHit) {
                        this.handleGalleryHit(ghb.el);
                        return;
                    }
                }
            }
        ...
        handleGalleryHit: function (entity) {
            // ギャラリーモデルヒット: アニメーションのみ（スタンプ取得なし）
            this.velocity.multiplyScalar(-0.6);
            this.velocity.y += 3;
            ...
            try {
                if (typeof window.playGalleryHitAnimation === 'function') {
                    window.playGalleryHitAnimation(entity);
                }
            } catch (e) {}
        }
    });
    ...
    // ---- ギャラリー当たり判定ボックスコンポーネント ----
    // allGalleryHitboxes に登録。ヒット時はアニメーションのみ（スタンプ取得なし）
    if (!AFRAME.components['gallery-hitbox'])
    AFRAME.registerComponent('gallery-hitbox', {
        schema: {
            width:  { type: 'number', default: 1 },
            height: { type: 'number', default: 1 },
            depth:  { type: 'number', default: 1 }
        ...
        init: function () {
            if (typeof window.allGalleryHitboxes !== 'undefined' && !window.allGalleryHitboxes.includes(this)) {
                window.allGalleryHitboxes.push(this);
            }
            this.box = new THREE.Box3();
            this.updateBox();
        },
        ...
        remove: function () {
            if (typeof window.allGalleryHitboxes !== 'undefined') {
                const index = window.allGalleryHitboxes.indexOf(this);
                if (index > -1) window.allGalleryHitboxes.splice(index, 1);
            }
        },
        ...
        updateBox: function () {
            ...
            const center = pos.clone();
            center.y += halfHeight; // 下端を原点として上方向にオフセット
