...
/**
 * ARstampRally202610 専用のスタンプ記録・景品交換コントローラー。
 *
 * 従来の共有API（/api/record-marker-scan 等）は CSRF・セッションが無効な
 * api グループで、閾値もサーバー側で検証されていなかったため、202610 は
 * これらの専用エンドポイント（web グループ = CSRF + セッション有効）へ
 * 切り替える。このクラスは202610の閾値（10種）をサーバー側で強制する。
 */
class StampRally202610Controller extends Controller
{
    /**
     * 景品交換に必要なマーカー種数。
     * クライアント側 js-prize.blade.php の PRIZE_EXCHANGE_THRESHOLD(10) と一致させること。
     */
